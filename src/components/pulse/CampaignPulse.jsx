...
/*
  Campaign Pulse — pared down to two product-styled cards on the campaign
  Dashboard tab: "The daily pulse" activity feed + "The pace" benchmark.
  Six demo day-states scrubbed via the fixed DEMO pill at the bottom.
...
// Survives remounts (the captured-HTML subtree can wipe and re-mount the block).
...
export default function CampaignPulse() {
  ...
  return (
    <div className="cp-root">
      {/* status lead — the page's one-line answer */}
      <div className="cp-lead" key={`lead-${scene.day}`}>
        <h2 className="cp-lead-headline">{scene.headline}</h2>
        <span className="cp-lead-updated">{scene.updated}</span>
      </div>

      {/* section heading, outside the panel (matches Invited Creators grammar) */}
      ...
      {/* one merged panel: feed left, pace right, hairline divider */}
      <div className="cp-panel" key={`panel-${scene.day}`}>
        <div className="cp-panel-feed">
          {scene.feed.map((f, i) => (
      ...
      <nav className="cp-scrubber" aria-label="Demo day selector">
